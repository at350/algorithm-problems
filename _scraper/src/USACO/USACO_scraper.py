...
def get_contest_from_link(link: str) -> str:
    contest = link['href'].replace(
        'index.php?page=', '').replace('results', '')
...
class USACOScraper(Scraper):
    ...
    def fetch_problem(self, link: str) -> dict:
        ...
        if contest:
            if contest not in self.data:
                ...
                problem_links = get_page_soup(
                    host + link['href']).find_all('a', string='View problem')

                for problem_link in problem_links:
                    problem_data = self.get_problems(
                        problem_link['href'], self.data[contest])
                    print(problem_data)
    ...
    def get_problems(self, link: str, data) -> dict:
        ...
        division_match = re.search(
            r'(Bronze|Silver|Gold|Platinum)', panel_text)
        ...
        if not division_match or not title_match:
            raise Exception(
                'Problem page format is not correct, need manual check')

        division = division_match.group(0)
        number = title_match.group(1)
        title = title_match.group(2)

        io_file = io_format_match.group(
            2) if io_format_match else 'interactive'
        if 'stdin' in io_file:
            io_file = 'stdio'
        else:
    ...
    def update_data(self) -> dict:
        results_links = get_page_soup(
            host + 'index.php?page=contests').select('a[href*="results"]')
...
if __name__ == '__main__':
    USACOScraper(data_file, host).run()
